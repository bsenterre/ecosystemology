...
ED = data.table::fread("occurrence.txt", header = TRUE, sep = "\t", dec = ".", encoding = "UTF-8", colClasses = 'character') %>% dplyr::select(!id)
...
EDmap$decimalLatitude <- as.numeric(EDmap$decimalLatitude)
EDmap$decimalLongitude <- as.numeric(EDmap$decimalLongitude) #251
EDmap <- EDmap[!is.na(EDmap$decimalLatitude) & !is.na(EDmap$decimalLongitude),]
EDmap <- EDmap[EDmap$decimalLatitude > 0 | EDmap$decimalLatitude < 0,] #251
...
ui <- dashboardPage(
  dashboardHeader(title= "Ecosystem taxonomy"),
  dashboardSidebar(
      ...
      tabItem(tabName = "ecoSpeciesList",
              fluidRow(
                ...
                #splitLayout(cellWidths = c("99%", "1%"),
                 #      box(selectInput(inputId="ecoSpeciesSelect", label="Select an eco-species from the list of accepted names", choices=ecoSpList, selected="Affleurement rocheux de quartzite de la zone des Forêts tropicales montagnardes d'Afrique de l'ouest [FR]"),
                  #         width = 12)
                   #    ),
    ...
    #rowSelected <- input$mySpList_rows_selected
      ...
      #  selEcoSpeciesID <- selRow$ecoSpeciesID
       ...
       if(file.exists(paste0("ecoSp_",ecoSpSelected_r(),".tif"))) {
         ...
         tmprasterPt <- terra::as.points(tmpraster, values=TRUE, na.rm=TRUE)
  ...
  #)
    ...
    output$myKBAStats <- renderDT({
      if(file.exists(paste0("ecoSp_",ecoSpSelected_r(),".tif"))) {
        inputraster <- terra::rast(paste0("ecoSp_",ecoSpSelected_r(),".tif")) #20200505100648
        tmpraster <- terra::aggregate(inputraster, fact=5, fun="max", na.rm=TRUE)
        tmpraster <- round(cellSize(tmpraster, unit="ha"), digits=1)
        KBAStats <- terra::extract(tmpraster, KBA, fun='sum', na.rm=TRUE)#, bind=TRUE)
        ...
        total <- round(terra::expanse(tmpraster, unit="ha"), digits = 0)
